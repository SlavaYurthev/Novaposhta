...
class NovaPoshta_Api2 {
	...
	function getArea($findByString = '', $ref = '') {
		...
		empty($this->areas) AND $this->areas = include dirname(__FILE__).'/NovaPoshta_Api2Areas.php';
	...
	function getTimeIntervals($cityRef) {
		return $this->request('Common', 'getTimeIntervals', array('RecipientCityRef' => $cityRef));
	}
	...
	protected function checkInternetDocumentRecipient(array & $counterparty) {
		...
		if ( ! isset($counterparty['FirstName']))
	 ...
	 * @param mixed
	 */
	function newInternetDocument($sender, $recipient, $params, $ref = false) {
	...
	function insertDocuments($refs = array()) {
		return $this->request('ScanSheet', 'insertDocuments', array('DocumentRefs' => $refs));
	}

	function removeDocuments($refs = array()) {
		return $this->request('ScanSheet', 'removeDocuments', array('DocumentRefs' => $refs));
	}
